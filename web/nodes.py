...
class TorNode(NodeMixin):
    def __init__(
        ...
        # NodeMixin already uses the read-only "size" property.
        # Store torrent file size using a different attribute.
        ...
        for i in res:
            ...
            if len(folders) > 1:
                ...
                for j in range(len(folders) - 1):
                    current_node = next(
                        (k for k in previous_node.children
                         if k.name == folders[j]),
                        None
                    )
        ...
        if i.is_folder:
            ...
            if i.name != ".unwanted":
                msg[0] += (
                    ...
                    f'name="foldernode_{msg[1]}"> '
                    f'<label for="{i.name}">{i.name}</label>'
                )
            ...
            if i.priority == 0:
                msg[0] += (
                    f'<input type="checkbox" '
                    f'name="filenode_{i.file_id}" '
                    f'data-size="{i.file_size}"> '
                    f'<label data-size="{i.file_size}" '
                    f'for="filenode_{i.file_id}">'
                    f'{i.name}</label> / {i.progress}%'
                )
            else:
                msg[0] += (
            ...
            msg[0] += (
                f'<input type="hidden" '
                f'value="off" name="filenode_{i.file_id}">'
            )
            msg[0] += "</li>"
